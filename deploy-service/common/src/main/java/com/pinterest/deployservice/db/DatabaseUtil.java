...
public class DatabaseUtil {
    ...
    public static BasicDataSource createMysqlDataSource(String host, int port,
        ...
        String url = String.format(
            "jdbc:mysql://%s:%d/deploy?connectTimeout=5000&socketTimeout=3000&characterEncoding=UTF-8&connectionCollation=utf8_general_ci",
            host, port);
        return createDataSource(MYSQL_JDBC_DRIVER, url, user, passwd, poolSize, MAX_WAIT_TIME_FOR_CONN_IN_MS);
    }
    ...
    public static BasicDataSource createMXJDataSource(String dbName, String baseDir, int port) {
        String url = String.format("jdbc:mysql:mxj://localhost:%d/%s?server.basedir=%s&createDatabaseIfNotExist=true&server.initialize-user=true",
            port, dbName, baseDir);
        return createDataSource(MYSQL_JDBC_DRIVER, url, "tester", "passwd", "0:8:8:0", MAX_WAIT_TIME_FOR_CONN_IN_MS);
    }
    ...
    public static BasicDataSource createDataSource(String driverClassName, String url,
        String user, String passwd, String poolSize, int maxWaitInMillis) {
